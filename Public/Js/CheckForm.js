...
*/
	...
	//主函数
	function CheckForm(data,oForm,target,ptarget)
	{
		
		var els =oForm[0].elements;
		//遍历所有表元素
		for(var i=0;i<els.length;i++)
		{
			  var parent =els[i].parentNode;
			  var warning=document.createElement("span");
			warning.className=target;
			//是否需要验证
			if(els[i].getAttribute("check"))
			{
				...
				if(!reg.test(sVal))
				{
					...
					if (target==undefined)
					{
						...
						var obj=document.getElementById(""+ptarget+"");
						if(obj!=null)return false;
						parent.parentNode.id=ptarget;
						//parent.parentNode.className=ptarget;
						warning.innerHTML=els[i].getAttribute("warning");
					    parent.insertBefore(warning,els[i].nextSibling );
					    els[i].addEventListener("blur",function(){
					    	var val=this.value;
					    	if(val!=''){
					    		//parent.parentNode.className="control-group";
			...
			}else if(els[i].getAttribute("checkSame")){
				//取得验证的正则字符串
				var name=els[i].getAttribute("checkSame");
