...
class TagLibHtml extends TagLib{
    ...
    protected $tags   =  array(
        ...
        'list'      =>  array('attr'=>'id,pk,pkName,style,action,actionlist,show,datasource,checkbox','close'=>0),
    ...
    public function _list($attr) {
        ...
        $datasource = $tag['datasource'];               //列表显示的数据源VoList名称
        $pk         = empty($tag['pk'])?'id':$tag['pk'];//主键名，默认为id
		$pkName     =empty($tag['pkname'])?'key':$tag['pkname'];//对认主键名input Name值
        $style      = $tag['style'];                    //样式名
        $name       = !empty($tag['name'])?$tag['name']:'vo';                 //Vo对象名
        $action     = $tag['action']=='true'?true:false; 
		$sort     = $tag['sort']=='true'?true:false;                   //是否显示功能操作
		$sortinput     = $tag['sortinput']=='true'?true:false;                   //是否显示input功能操作
        $key         =  !empty($tag['key'])?true:false;                
        ...
        if($sortinput){
        $order=explode(':',$show[0]);
        $orderName=explode('|',$order[1]);
		...
		if(!empty($checkbox) && 'true'==strtolower($checkbox)) {//如果指定需要显示checkbox列
            $parseStr .='<th width="20"><input type="checkbox" id="check" onclick="CheckAll(\''.$id.'\')"></th>';
        }
		
        if(!empty($key)) {
            $parseStr .= '<th width="20">No</th>';
        }
		if($sortinput){
			$parseStr .= '<th width="40">'.$orderName[0].'</th>';
		}
        foreach($fields as $field) {//显示指定的字段
            $property = explode('|',$field[0]);
            $showname = explode('|',$field[1]);
            if(isset($showname[1])) {
                $parseStr .= '<th width="'.$showname[1].'" align="center">';
            }else {
                $parseStr .= '<th>';
            }
            ...
            if($sort) {
                $parseStr .= '<a href="javascript:sortBy(\''.$property[0].'\',\'{$sort}\',\''.ACTION_NAME.'\')" title="按照'.$showname[2].'{$sortType} ">'.$showname[0].'<eq name="order" value="'.$property[0].'" ><i class="{$sortImg}"></i></eq></a></th>';
            }else{
                $parseStr .= $showname[0].'</th>';
            }
        ...
        $parseStr .= '<volist name="'.$datasource.'" id="'.$name.'" mod="2" ><tr <eq name="mod" value="1"> class="row odd"</eq><eq name="mod" value="0"> class="row"</eq>';	//支持鼠标移动单元行颜色变化 具体方法在js中定义
        $parseStr .= '>';
        if(!empty($checkbox)) {//如果需要显示checkbox 则在每行开头显示checkbox
            $parseStr .= '<td><input type="checkbox" name="'.$pkName.'"	value="{$'.$name.'.'.$pk.'}"></td>';
        }
       
		if($sortinput){//如果需要显示checkbox 则在每行开头显示checkbox
              $parseStr .= '<td><input type="text" name='.$order[0].'	value="{$'.$name.'.'.$order[0].'}"  class="input-smini text-center"></td>';
		}
        if(!empty($key)) {
            $parseStr .= '<td>{$i}</td>';
        }
                ...
                if(count($href)>1) {
                    ...
                    if(count($array)>1) {
						...
						if(count($tmp)>=1){
							...
							$temp[]='\'{$'.$name.'.'.$hre.'|addslashes}\'';	
						}else{
							$temp[]='\'{$'.$name.'.'.$href[1].'|addslashes}';
						}
                        $parseStr .= '<a href="javascript:'.$href[0].'('.implode(',',$temp).')">';
                    }
